...
with col1:
    # 지도 HTML/JS
    # 클릭 시 주소창(URL)에 좌표와 PNU를 담아 리로드 시키는 방식
    map_html = f"""
    <div id="map" style="width:100%;height:650px;border-radius:10px;border:1px solid #ccc;"></div>
    <div id="v-status" style="padding:5px; font-size:12px; color:gray;">필지를 클릭하세요.</div>

    <script type="text/javascript" src="https://dapi.kakao.com/v2/maps/sdk.js?appkey={MAP_JS_KEY}&libraries=services"></script>
    <script>
        var map, currentPoly;
        ...
        kakao.maps.event.addListener(map, 'click', function(e) {{
            geocoder.coord2Address(e.latLng.getLng(), e.latLng.getLat(), function(res, stat) {{
                if (stat === kakao.maps.services.Status.OK && res[0].address) {{
                    ...
                    // 주소창의 파라미터를 변경하여 페이지 리로드 (Streamlit 파이썬 서버로 데이터 전달)
        ...
        // 하이라이트 그리기 (URL에 pnu가 있을 때 브라우저가 직접 Vworld 호출)
